import React from 'react';

const Terms: React.FC = () => {
	return (
		<section className='mt6'>
			<h1 className='moon-gray'>Terms</h1>
			<div className='mt3'>
				<ul className='measure-wide center mv3 mh4 pv3 ph5-l ph4-m ph3-ns light-green b f3'>
					<li>
						<strong>Please remember that this is a chill site.</strong> Join and start sharing your inspiration with your friends, express yourself and always be U.{' '}
					</li>
					<li>
						<strong>Please invite your friends to join Uprophet and help increase our growing community.</strong>
					</li>
				</ul>
			</div>
		</section>
	);
};

export default Terms;
